...
def get_author_books(chosen_author):
    ...
    print(authorQuery)
    #Debug
    print(type(authorQuery))
    #Works with full string queries, not substring queries
    query = library.query.filter(authorQuery).all()
    print(type(query))
    print(len(query))
    for title in query:
        ouevre.append(library.serialize(title))
        print(type(title))
    print(type(ouevre))
